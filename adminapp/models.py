from django.db import models


# Create your models here.
class Admin(models.Model):
    id: models.AutoField(primary_key=True)
    username = models.CharField(max_length=30, blank=False, unique=True)
    password = models.CharField(max_length=12, blank=False)

    class Meta:
        db_table="rs,_table"
class Register(models.Model):
    id: models.AutoField(primary_key=True)
    name = models.CharField(max_length=50, blank=False)
    address = models.CharField(max_length=50, blank=False)
    email = models.CharField(max_length=40, blank=False, unique=True)
    phno = models.CharField(max_length=15, blank=False, unique=True)
    username = models.CharField(max_length=30, blank=False, unique=True)
    password = models.CharField(max_length=10, blank=False)
    class Meta:
        db_table="register_table"

class Packages(models.Model):
    id = models.AutoField(primary_key=True)
    tourcode = models.CharField(max_length=20, blank=False)
    tourname = models.CharField(max_length=40, blank=False)
    tourpackage = models.CharField(max_length=40, blank=False)
    desc = models.CharField(max_length=40, blank=False)
    class Meta:
        db_table = "package_table"

class ContactFormEntry(models.Model):
    name = models.CharField(max_length=100)
    email = models.EmailField()
    problem = models.CharField(max_length=100)
    message = models.TextField()

    class Meta:
        db_table="contact"
